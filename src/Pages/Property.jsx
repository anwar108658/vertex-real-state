    ...
    <>
        <div className='max-w-[1200px] mx-auto !overflow-hidden scrollbar-hide'>
            <div className='h-[80vh] flex gap-3'>
                <div className='flex-5'><img className='w-full h-full object-cover hover:brightness-90 cursor-pointer rounded-lg' src={item?.image[0]} alt="im" /></div>
                <div className='flex-2 grid grid-rows-2 gap-3'>
                    <div><img className='w-full h-full object-cover hover:brightness-90 cursor-pointer rounded-lg' src={item?.image[1]} alt="" /></div>
                    <div className='  grid  grid-rows-2 gap-3'>
                        <img className='w-full h-full object-cover hover:brightness-90 cursor-pointer object-center rounded-lg' src={item?.image[2]} alt="" />
                        <img className='w-full h-full object-cover hover:brightness-90 cursor-pointer object-center rounded-lg' src={item?.image[3]} alt="" />
                    </div>
                </div>
            </div>
        ...
        <div className='w-full h-screen bg-black/60 backdrop-blur-sm fixed top-0 left-0 z-50 flex items-center justify-center'>
            <div className='relative w-full max-w-4/5 h-[90vh] flex flex-col gap-3 p-5 px-7 bg-white rounded-lg'>
                <button className='absolute text-[1rem] flex items-center top-2 right-2 cursor-pointer'>
                    <FontAwesomeIcon size='md' color='red' icon={faXmark}/>
                </button>
                <Segment size='sm' className='w-full' options={popup}/>
                <div className='flex-1 grid grid-cols-2 gap-3 overflow-auto'>
                    {item?.image.length > 0 && item.image.map((img, index) => (
                        <img key={index} className='w-full h-[400px] cursor-pointer rounded-lg' src={img} alt={`Image ${index}`} />
                    ))}
                </div>
                <div className='flex justify-between items-center'>
